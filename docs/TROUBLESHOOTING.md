...
| Skill does not record what I did | Capture has two layers that never silence each other: (1) precision touch capture reads the raw touchscreen stream (`getevent -t`) through a shell-identity server - built into the app since v2.1 (Skills screen → "Set up (built-in, no other app)": pair once with Wireless debugging, ~1 minute, Android 11+) or via the optional Shizuku app; (2) app-event capture (buttons, typing, scrolls, app switches) works on all Android versions. While recording, every tap and drag is drawn as red ink on screen - if you see ink, precision capture is live; if the REC card says "App-event capture", taps in apps that hide from accessibility may be missed (add them as steps in the editor). If nothing moves at all, check JARVIS is enabled in Settings → Accessibility (the REC card warns when it is off). JARVIS's own actions and the system shade are never recorded. |
