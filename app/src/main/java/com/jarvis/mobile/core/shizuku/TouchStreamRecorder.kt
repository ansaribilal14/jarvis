...
/**
 * Live `getevent -t` touch-stream recorder running through the Shizuku
 * UserService shell ([ShizukuShell]). This is the layer that finally makes
 * "record every single click" TRUE on every device: the kernel touchscreen
 * stream carries every contact with real coordinates, in every app, whether
 * or not the app emits accessibility events. Reading it never consumes a
 * single touch - the user interacts 100% normally while recording.
 */
object TouchStreamRecorder {
    ...
    @Volatile private var handle: ShizukuShell.StreamHandle? = null
    ...
    fun start(): Boolean {
        ...
        scope.launch {
            try {
                val probe = ShizukuShell.exec("getevent", "-pl")
                    ?: throw IllegalStateException("device probe failed (shizuku not ready?)")
                ...
                val stream = ShizukuShell.stream("getevent", "-t", device.path)
    ...
    fun stop() {
        ...
        handle?.stop() // closes the pipe (unblocks the reader) + kills the shell process
        handle = null
        decoder = null
    }
